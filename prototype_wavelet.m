...
helperCWTTimeFreqPlot(dpoaeCWT,t.*1000,f,...
    'image','CWT of OAE','milliseconds','Hz')
...
saveas(figure(1),'daily_click_data.jpg');
...
for k=1:3    
    for j=1:20 % 7 days 
        ...
        if (j==1|j==11|j==12)
            saveas(temp_fig,sprintf('./magnitude/train/cat.day%d.click.%s.jpg', j, w_type(k)));
        elseif (j==20)
            saveas(temp_fig,sprintf('./magnitude/test/cat.day%d.click.%s.jpg', j, w_type(k)));                
        elseif (13 <= j) && (j <= 19)
            saveas(temp_fig,sprintf('./magnitude/test/dog.day%d.click.%s.jpg', j, w_type(k)));                
        else
            saveas(temp_fig,sprintf('./magnitude/train/dog.day%d.click.%s.jpg', j, w_type(k)));
        end        
    end    
end

%  magnitude : daily conversion transform
for k=1:3    
    for j=1:20 % 7 days 
        temp_fig = figure;
        [temp_cwt, temp_freq] = cwt(normalize(daily_conversion.data(:,j+1)), w_type(k));
        args = {daily_click.data(:,1), temp_freq, abs(temp_cwt).^2};
        surf(args{:},'edgecolor','none');
        view(0,90);
        axis tight;
        ...
        if (j==1|j==11|j==12)
            saveas(temp_fig,sprintf('./magnitude/train/cat.day%d.conv.%s.jpg', j, w_type(k)));
        elseif (j==20)
            saveas(temp_fig,sprintf('./magnitude/test/cat.day%d.conv.%s.jpg', j, w_type(k)));                
        elseif (13 <= j) && (j <= 19)
            saveas(temp_fig,sprintf('./magnitude/test/dog.day%d.conv.%s.jpg', j, w_type(k)));                
        else
            saveas(temp_fig,sprintf('./magnitude/train/dog.day%d.conv.%s.jpg', j, w_type(k)));
        end
    end    
end

% real : daily click transform
for k=1:3    
    for j=1:(click_c-1)  
        temp_fig = figure;
        [temp_cwt, temp_freq] = cwt( normalize(daily_click.data(:,j+1), 'range') , w_type(k));
        ...
        h = colorbar;
        h.Label.String = 'Power';
        saveas(temp_fig,'mag.day%d.click.%s.jpg', j, w_type(k));
    end    
end
...
for k=1:3    
    for j=1:(conversion_c-1)  
        ...
        h = colorbar;
        h.Label.String = 'Power';
        saveas(temp_fig,'mag.day%d.conversion.%s.jpg', j, w_type(k));
    end    
end
